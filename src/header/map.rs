...
/// use request_rs::headers::HeaderMap;
/// use request_rs::headers::{HOST, CONTENT_LENGTH, LOCATION, HeaderMap};
/// use request_rs::headers::{HOST, CONTENT_LENGTH, LOCATION};
...
impl<T> HeaderMap<T> {
    ...
    /// ```
    /// # use request_rs::headers::HeaderMap;
    /// # use request_rs::headers::{ACCEPT, HOST};
    ...
    ///
    /// ```
    /// # use request_rs::headers::{HeaderMap, ACCEPT, HOST};
    /// # use request_rs::headers::{ACCEPT, HOST};
    /// let mut map = HeaderMap::new();
    ///
    ...
    ///
    /// ```
    /// # use request_rs::headers::{HeaderMap, HOST};
    /// let mut map = HeaderMap::new();
    ///
    /// assert!(map.is_empty());
    ///
    ...
    /// ```
    /// # use request_rs::headers::HeaderMap;
    /// # use  request_rs::headers::{CONTENT_LENGTH, HOST};
    /// let mut map = HeaderMap::new();
    /// use  request_rs::headers::{CONTENT_LENGTH, HOST};
    /// map.insert(HOST, "hello".parse().unwrap());
    /// map.append(HOST, "goodbye".parse().unwrap());
...
impl<'a, K, V, T> TryFrom<&'a HashMap<K, V>> for HeaderMap<T>
    where
        ...
        <HeaderName as TryFrom<&'a K>>::Error: Into<crate::error::Error>,
        T: TryFrom<&'a V>,
        T::Error: Into<crate::error::Error>,
{
    type Error = Error;

    fn try_from(c: &'a HashMap<K, V>) -> Result<Self, Self::Error> {
        c.into_iter()
            .map(|(k, v)| -> crate::error::Result<(HeaderName, T)> {
...
impl<'a, T> VacantEntry<'a, T> {
    ...
    ///
    /// ```
    /// # use  request_rs::headers::{HeaderMap, Entry};
    /// let mut map = HeaderMap::new();
    ///
...
impl<'a, T> OccupiedEntry<'a, T> {
    ...
    ///
    /// ```
    /// # use  request_rs::headers::{HeaderMap, Entry, HOST};
    /// let mut map = HeaderMap::new();
    /// map.insert(HOST, "world".parse().unwrap());
    ///

//! HTTP request types.
//!
//! This module contains structs related to HTTP request_rs, notably the
//! `Request` type itself as well as a builder to create request_rs. Typically
...
impl Request<()> {
    ...
    pub fn get<T>(uri: T) -> Builder
        where
            Url: TryFrom<T>,
            <Url as TryFrom<T>>::Error: Into<crate::error::Error>,
...
impl<T> Request<T> {
    ...
    /// ```
    #[inline]
    pub fn uri_mut(&mut self) -> Option<&mut Url> {
        self.head.uri.as_mut()
    }
    ...
    /// ```
    /// use request_rs::produce::*;
    /// use request_rs::headers::{HeaderValue, HOST};
    /// let mut request: Request<()> = Request::default();
...
impl Builder {
    ...
    pub fn method<T>(self, method: T) -> Builder
        where
            Method: TryFrom<T>,
            <Method as TryFrom<T>>::Error: Into<crate::error::Error>,
    {
    ...
    {
        self.and_then(move |mut head| {
            head.uri = Some(TryFrom::try_from(uri).map_err(Into::into)?);
            Ok(head)
        })
    }
    ...
    /// ```
    pub fn uri_ref(&self) -> Option<&Url> {
        match self.inner.as_ref().ok().take() {
            Some(part) => {
                part.uri.as_ref()
            }
            None => None
    ...
    /// response.add_header("X-Custom-Foo", "bar");
    /// response.add_header("content-length", 0);
    /// response.body(()).unwrap();
    /// ```
    pub fn add_header<K, V>(&mut self, key: K, value: V) -> Result<()>
        where
            HeaderName: TryFrom<K>,
            <HeaderName as TryFrom<K>>::Error: Into<crate::error::Error>,
            HeaderValue: TryFrom<V>,
    ...
    /// ```
    /// use request_rs::produce::*;
    /// use request_rs::headers::HeaderValue;
    /// let mut req = Request::builder();
    /// {

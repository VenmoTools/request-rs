...
use crate::error::{Error, InvalidHttpVersion};
...
impl FromStr for Version {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "HTTP/0.9" => Ok(Version(Http::Http09)),
            "HTTP/1.0" => Ok(Version(Http::Http10)),
            "HTTP/1.1" => Ok(Version(Http::Http11)),
            "HTTP/2.0" => Ok(Version(Http::H2)),
            "HTTP/3.0" => Ok(Version(Http::H3)),
            other => Err(Error::from(InvalidHttpVersion::new(other)))
        }
    }
}
